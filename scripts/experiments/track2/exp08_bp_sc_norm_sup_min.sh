#!/usr/bin/env bash
# Track 2 (projection) — 4-fold CV — SupCon MIN — bp_sc_norm — no axis rotation
# Usage: sbatch --array=0-3 scripts/experiments/track2/exp08_bp_sc_norm_sup_min.sh
#SBATCH --job-name=t2-bpscn-supmin
...
TRACK=configs/track2_sup_min.yaml
...
mkdir -p logs
GROUP_NAME="t2-exp08-bp-sc-norm-supmin"
RUN_NAME="t2-exp08-bp-sc-norm-supmin-train${TRAIN}-val${VAL}"
