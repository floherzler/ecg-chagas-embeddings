...
def get_parser():
    ...
    parser.add_argument(
        "-i", "--signal_files", type=str, required=True, nargs="*"
    )  # exams_part0.hdf5, exams_part1.hdf5, ...
    parser.add_argument(
        "-d", "--demographics_file", type=str, required=True
    )  # exams.csv
    parser.add_argument(
        "-l", "--labels_file", type=str, required=True
    ...
    parser.add_argument(
        "--log_skips",
        action="store_true",
        help="If set, log a few skipped records without breaking tqdm bars.",
    )
    parser.add_argument(
        "--max_skip_logs",
        type=int,
        default=25,
        help="Maximum number of per-record skip logs to print when --log_skips is set.",
    )
...
def run(args):
    ...
    for idx, row in df.iterrows():
        ...
        )  # This variable was encoding as a binary value.
    ...
    if len(args.output_paths) == len(args.signal_files):
        ...
        raise Exception(
            "The number of signal files must match the number of output paths."
        )

    num_groups = len(signal_files)

    # Iterate over the input signal files.
    for k in tqdm(range(num_groups), desc="CODE-15 parts"):
        ...
        with h5py.File(signal_file, "r") as f:
            ...
            processed = 0
            skipped_no_label = 0
            skipped_empty_after_trim = 0
            skip_logs = 0

            # Iterate over the exam IDs in each signal file.
            pbar = tqdm(
                range(num_exam_ids),
                desc=f"Exams ({os.path.basename(signal_file)})",
                leave=False,
            )
            for i in pbar:
                exam_id = exam_ids[i]
                ...
                if r >= s:
                    skipped_empty_after_trim += 1
                    if args.log_skips and skip_logs < args.max_skip_logs:
                        tqdm.write(
                            f"[skip] exam_id={exam_id} reason=empty_after_trim (r={r}, s={s})"
                        )
                        skip_logs += 1
                    continue
                else:
                ...
                digital_signals = np.clip(
                    digital_signals, -(2 ** (num_bits - 1)) + 1, 2 ** (num_bits - 1) - 1
                )
                ...
                if (i + 1) % 1000 == 0:
                    pbar.set_postfix(
                        ...
                        skipped_empty=skipped_empty_after_trim,
                    )

            tqdm.write(
                "[summary] "
                f"{os.path.basename(signal_file)} processed={processed} "
                f"skipped_no_label={skipped_no_label} "
                f"skipped_empty_after_trim={skipped_empty_after_trim}"
            )
