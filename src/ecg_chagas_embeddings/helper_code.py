...
def get_label(string, allow_missing=True):
...
def compute_challenge_score(
    ...
    # Print a simple confusion matrix at the optimal threshold with nicer formatting
    tqdm.write("      | Pred 1 | Pred 0 |")
    tqdm.write("------+--------+--------+")
    tqdm.write(f"True 1| {tp:6} | {fn:6} |")
    tqdm.write("------+--------+--------+")
    tqdm.write(f"True 0| {fp:6} | {tn:6} |")
    tqdm.write("------+--------+--------+")
...
def compute_f_measure(labels, outputs):
    ...
    tp, fp, fn, _tn = A[0, 0], A[0, 1], A[1, 0], A[1, 1]
    if 2 * tp + fp + fn > 0:
