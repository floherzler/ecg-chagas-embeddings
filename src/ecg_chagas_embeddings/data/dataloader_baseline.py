...
class ECGDatasetH5:
    def __init__(
        ...
        self.traces = traces
        self.exams = exams
        self.sources = f[source_dset]
        self.ages = f[age_dset]
        self.sexes = f[sex_dset]
        self.ids = ids  # [0,1,2,...]
        ...
        self.chagas = chagas_df[chagas_diag].to_numpy(
            float
        )  # chagas diagnos -- missing ones are replaced by 'nan'

        # weights due to unbalance
        self.pos_weights = torch.tensor(
            np.nansum(1 - self.chagas) / np.nansum(self.chagas), dtype=torch.float32
        )
    ...
    def getbatch(self, start=0, end=None, attr_only=False):
        ...
        if attr_only:
            ...
            return torch.tensor(self.traces[indices], dtype=torch.float32).transpose(
                -1, -2
...
class ECGDataloaderH5:
    def __init__(
        self, dset, batch_size, start_idx=0, end_idx=None, cont_from_start=False
    ):
        if end_idx is None:
            end_idx = len(dset)
        self.dset = dset
    ...
    def __next__(self):
        ...
        # CAREFUL: added by Flo
        if self.drop_last and (end - self.start) < self.batch_size:
            raise StopIteration
