#!/usr/bin/env bash
# Track 2 (projection) — 4-fold CV — SupCon STANDARD (no imbalance tweaks) — bp — WITH axis rotation
# Usage: sbatch --array=0-3 scripts/experiments/track2/exp02_bp_sup_standard_rot.sh
#SBATCH --job-name=t2-bp-supstd-r
...
mkdir -p logs
GROUP_NAME="t2-exp02-bp-supstd-rot"
RUN_NAME="t2-exp02-bp-supstd-rot${ROT_DEG}-train${TRAIN}-val${VAL}"
